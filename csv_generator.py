...
for key in df.keys():
    ...
    if (type(float()) == column.dtypes).any() or (type(int()) == column.dtypes).any():
        ...
        new_data = pd.concat([new_data,df[key]],axis=1)
...
for key in df.keys():
    ...
    if  key in good_columns:
        ...
        mean = float(pd.stats.moments.expanding_mean(column).irow(-1))
        st_d = float(pd.stats.moments.expanding_std(column).irow(-1))
        skew = float(pd.stats.moments.expanding_skew(column).irow(-1))
        kurtorsis = float(pd.stats.moments.expanding_kurt(column).irow(-1))
        ...
        if type(float()) == column.dtypes: #this is fine because we are assured only a single column
